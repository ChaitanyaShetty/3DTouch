...
-(void)application:(UIApplication *)application performActionForShortcutItem:(UIApplicationShortcutItem *)shortcutItem completionHandler:(void (^)(BOOL))completionHandler
{
    ...
    if ([shortcutItem.type isEqualToString:@"dynamic1"]) {
        ...
        UIViewController *controller = [board instantiateViewControllerWithIdentifier:@"ViewController"];
        self.window.rootViewController = controller;
    } else if ([shortcutItem.type isEqualToString:@"dynamic2"]) {
        UIStoryboard *board = [UIStoryboard storyboardWithName:@"Main" bundle:nil]; 
        UIViewController *controller = [board instantiateViewControllerWithIdentifier:@"ViewController2"];
        self.window.rootViewController = controller;
    } else if ([shortcutItem.type isEqualToString:@"dynamic3"]) {
        UIStoryboard *board = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
        UIViewController *controller = [board instantiateViewControllerWithIdentifier:@"ViewController3"];
        self.window.rootViewController = controller;
    }
...
- (NSPersistentContainer *)persistentContainer {
    ...
    @synchronized (self) {
        if (_persistentContainer == nil) {
            _persistentContainer = [[NSPersistentContainer alloc] initWithName:@"_D_Touch"];
